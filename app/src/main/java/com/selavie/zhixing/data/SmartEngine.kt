...
class SmartEngine {
    fun parseCapture(text: String, today: LocalDate = LocalDate.now()): CaptureDraft {
        ...
        val reason = when (type) {
            ...
            ItemType.NOTE -> "未识别到明确行动，建议保存为笔记"
        }
    ...
    fun taskPhase(task: TaskItem, now: LocalDateTime = LocalDateTime.now()): TaskPhase {
        ...
        val end = start.plusMinutes(task.estimateMinutes.coerceIn(1, 1440).toLong())
        return when {
    ...
    fun ask(question: String, data: AppData): AssistantReply {
        ...
        if (data.preferences.includeNotesInAi) data.notes.forEach { note ->
            score(question, note.title + note.content + note.tags.joinToString()).takeIf { it > 0 }?.let {
                sources += Citation(note.id, note.title, "笔记", note.content.take(88), note.updatedAt)
            }
        }
        ...
        if (ranked.isEmpty()) return AssistantReply("现有数据不足以回答这个问题。我没有找到可引用的笔记、任务或目标；可以先记录相关信息，或换一个更具体的关键词。")
    ...
    fun dailyReview(data: AppData, date: LocalDate): String {
        ...
        val totalMinutes = dayTasks.sumOf { it.estimateMinutes.coerceIn(1, 1440) }
        return buildString {
